...
<div class="container px-4">
    <div class="bg-white p-5 mt-5 rounded-lg">
        <div class="flex">
            <h2 class="text-gray-600 font-bold">Input Data Dosen</h2>
        </div>
        ...
        <form action="{{ route('store_dosen') }}" method="POST" class="w-full mt-5">
            ...
            <div class="mt-3">
                ...
                <div class="border-2 p-1 @error('no_telp') border-red-400 @enderror">
                    <input name="no_telp" value="{{ old('no_telp') }}" class="text-black w-full h-full focus:outline-none text-sm" id="no_telp" type="no_telp">
                </div>
                @error('no_telp')
                    <p class="italic text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror

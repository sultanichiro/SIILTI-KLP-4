...
@if (session('message'))
   <div id="toast-container" class="fixed z-50 items-center w-full max-w-xs p-4 text-gray-500 bg-white border-l-2 border-green-400 shadow top-5 right-5" role="alert">
        ...
        <table class="w-full mt-3 text-sm text-gray-600">
            ...
            <tbody>
                ...
                @foreach ($loans as $loan)
                    <tr class="border-b p-2 {{ $loan->returned ? 'bg-green-100' : '' }}">
                        ...
                        <td class="p-2 flex gap-2">
                            @if (!$loan->returned)
                                <a href="/saran-barang-user/{{ $loan->id }}" class="bg-orange-500 py-1 px-4 rounded text-white">
